...
path_checklist = project_dir / "process_checklist_mid.csv"
path_notes = project_dir / "notes_mid.json"
...
# 使用 list() 以免在循環中 pop 報錯
for idx, note in enumerate(list(st.session_state.temp_notes_mid)):
    with cols[idx % 3]:
        with st.container(border=True):
            c1, c2 = st.columns([4, 1])
            with c1:
                # 元件 Key 包含專案名稱，確保在不同專案間切換時 ID 唯一
                new_title = st.text_input(
                    f"標題_{idx}", 
                    value=note["title"], 
                    key=f"title_{idx}_{curr_proj}_mid", 
                    label_visibility="collapsed"
                )
            with c2:
                if st.button("🗑️", key=f"del_{idx}_{curr_proj}_mid"):
            ...
            new_content = st.text_area(
                f"內容_{idx}", 
                value=note["content"], 
                key=f"content_{idx}_{curr_proj}_mid", 
                height=120, 
                label_visibility="collapsed"
            )
