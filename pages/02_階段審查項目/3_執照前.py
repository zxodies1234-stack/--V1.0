...
# --- 1. 匯入新架構模組 ---
# 注意：在子頁面中不再呼叫 st.set_page_config 與 render_sidebar
...
path_checklist = project_dir / "process_checklist_pre.csv"
path_notes = project_dir / "notes_pre.json"
...
for idx, note in enumerate(list(st.session_state.temp_notes_pre)):
    with cols[idx % 3]:
        with st.container(border=True):
            ...
            with c1:
                new_title = st.text_input(
                    f"標題_{idx}", 
                    value=note["title"], 
                    key=f"title_{idx}_{curr_proj}_pre", 
                    label_visibility="collapsed"
                )
            with c2:
                if st.button("🗑️", key=f"del_{idx}_{curr_proj}_pre"):
            ...
            new_content = st.text_area(
                f"內容_{idx}", 
                value=note["content"], 
                key=f"content_{idx}_{curr_proj}_pre", 
                height=120, 
                label_visibility="collapsed"
            )
